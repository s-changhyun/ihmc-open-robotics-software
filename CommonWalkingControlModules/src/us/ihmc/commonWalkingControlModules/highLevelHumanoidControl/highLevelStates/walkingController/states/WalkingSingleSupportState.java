...
public class WalkingSingleSupportState extends SingleSupportState
{
   ...
   public void switchToToeOffIfPossible(RobotSide supportSide)
   {
      if (feetManager.doToeOffIfPossibleInSingleSupport() && feetManager.isInFlatSupportState(supportSide))
      {
         ...
         boolean pastMinimumTime = getTimeInCurrentState() > minimumTimeRatio * walkingMessageHandler.getNextSwingTime();

         boolean doToeOff = feetManager.checkIfToeOffSafeSingleSupport(nextFootstep, desiredCMP, currentICP, desiredICP, balanceManager.isOnExitCMP());
